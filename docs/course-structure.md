...
- Three assessment components, stated on the homepage with a link to the
  ...
  syncs: attendance and completion (48% in the underlying data; a single
  entry covering all twelve Weekly Syncs, see "Assessment weights and the
  pass/fail model" below).
- Graded CRS (pass/fail), does not count toward GPA. The outcome is decided
  by the pass rule in "Assessment weights and the pass/fail model" below,
  not by the percentage weights.
- IMPORTANT: before writing the CRS grade type to the homepage, verify the
  template's fixed grading/marks model actually permits a CRS/pass-fail
  grade type. The spec says SlopU's "name, marks and palette stay as they
  arrived," so a custom grade scheme that conflicts with the fixed
  university reads as ignoring the brief. If the template doesn't support
  CRS, flag it and stop rather than inventing one.
...
- The pass rule, stated plainly wherever assessment is discussed: a
  ...
  reach for one. Every individual assessment page states this rule in full;
  the main Assessment page's intro states only the CRS pass/fail fact, not
  the full rule.
- The percentage weights (22/30/48) stay in the frontmatter and data only
  because the platform's schema requires a numeric weight per assessment
  and needs them to sum to 100% to build; they are not a mark the student
  is chasing, and the pass/fail outcome is governed by the pass rule above,
  not by the weights.
- Weekly syncs are graded as one assessment entry worth 48% overall, not as
  ...
  entry's own page, not in the assessment collection's structure.
- All three assessment entries (Assignment 1, Assignment 2, Weekly syncs)
  hide their weight on screen (`hideWeight: true` in frontmatter). No
  assessment page or index shows a percentage or weight on screen,
  anywhere; the weight stays in the data only for the platform's
  sum-to-100 build check.
- The Weekly Syncs assessment page leads with the 8-of-12 threshold, since
  that threshold is the thing that actually decides whether a student
  passes. It explains, in plain English, what a weekly sync actually is,
  that you pass one by showing up, engaging and finishing the in-class task
  through Canvas, and that a missed sync can only be made up through the
